	...
	public static void main(String[] args) {
		...
		try {
			...
			for (Status s: mentionedStatuses) {
				...
				System.out.println("@" + s.getUser().getName() + " tweeted: " + s.getText()		// Getting the tweet's text.
									.toString().replaceAll("https://t.co/[a-zA-Z_0-9]*","")); 	// Here, we remove the link after tweets with media.
				
				if(s.getMediaEntities().length > 0) {
					System.out.println("Included media: ");
					
					for(int i = 0; i < s.getMediaEntities().length; i++) {
						if(s.getMediaEntities()[i].getType().equals("video")) System.out.println(s.getMediaEntities()[i].getVideoVariants()[0].getUrl()); // We get the video in its mp4 variant
